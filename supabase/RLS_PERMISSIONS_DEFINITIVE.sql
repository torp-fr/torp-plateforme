-- =====================================================
-- SYSTÈME DE PERMISSIONS RLS COMPLET ET DÉFINITIF
-- =====================================================
-- Création d'un système de permissions granulaires basé sur les types d'utilisateurs
-- =====================================================
...
SELECT
  auth.uid() as user_id,
  auth.get_user_type() as type_utilisateur;
...
FOR SELECT
TO authenticated
USING (auth.get_user_type() = 'super_admin');
...
FOR INSERT
TO authenticated
WITH CHECK (auth.get_user_type() = 'super_admin');
...
-- MAIS ne peut pas changer son user_type
...
WITH CHECK (
  ...
  AND (
    ...
    OR auth.get_user_type() IN ('admin', 'super_admin')
  )
);
...
TO authenticated
USING (
  auth.get_user_type() = 'super_admin'
  AND id != auth.uid()
);
...
- La fonction auth.get_user_type() utilise SECURITY DEFINER pour éviter la récursion
...
-- Fin du script
SELECT '✅ Système de permissions RLS configuré avec succès!' as status;
