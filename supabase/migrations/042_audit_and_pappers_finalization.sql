-- ============================================
-- MIGRATION 042: AUDIT & PAPPERS FINALIZATION
-- Created: 2026-02-12
-- Purpose: Complete RLS policies, links, and final setup
-- ============================================
...
CREATE TABLE IF NOT EXISTS audit_events (
  ...
  -- Event details
  event_type TEXT NOT NULL,                  -- 'api_call', 'criteria_eval', 'score_generated'
  event_category TEXT,                      -- 'upload', 'analysis', 'export'
...
CREATE TABLE IF NOT EXISTS analysis_results (
  ...
  -- References
  ccf_id UUID NOT NULL REFERENCES ccf(id) ON DELETE CASCADE,
  ...
  -- Scores by axis (5 main axes)
  enterprise_score INTEGER,                 -- ENTREPRISE (250 pts)
  enterprise_grade TEXT,

  price_score INTEGER,                      -- PRIX (300 pts)
  price_grade TEXT,

  completeness_score INTEGER,               -- COMPLETUDE (200 pts)
  completeness_grade TEXT,

  conformity_score INTEGER,                 -- CONFORMITE (150 pts)
  conformity_grade TEXT,

  delays_score INTEGER,                     -- DELAIS (100 pts)
  delays_grade TEXT,
  ...
  recommendations TEXT[],                   -- Top recommendations

  -- References used
  kb_references JSONB,                      -- Knowledge base references
  criteria_used TEXT[],                     -- Which criteria were evaluated
  data_sources TEXT[],                      -- Pappers, devis, etc

  -- Metadata
  analysis_type TEXT DEFAULT 'full',        -- 'full', 'quick', 'custom'
  ai_model TEXT,                            -- Model used for analysis
...
CREATE POLICY IF NOT EXISTS "Users see analysis results for their CCFs"
...
CREATE POLICY IF NOT EXISTS "Service role can insert analysis results"
...
LEFT JOIN ccf c ON ar.ccf_id = c.id
...
-- Grant SELECT on all audit views to authenticated users (limited by RLS)
GRANT SELECT ON get_ccf_analysis_history TO authenticated;
GRANT SELECT ON get_analysis_api_calls TO authenticated;
GRANT SELECT ON get_analysis_criteria_breakdown TO authenticated;
GRANT SELECT ON get_execution_summary TO authenticated;

-- Grant functions
GRANT EXECUTE ON FUNCTION get_execution_trace(TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION is_pappers_data_valid(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION assess_company_solvency(UUID) TO authenticated;

-- ============================================
-- MIGRATION COMPLETE
-- ============================================
-- Enhancements:
--   - Linked audit tables to quote_uploads and devis
--   - Added audit_events table for system events
--   - Added analysis_results table for aggregated results
--   - Created complete_analysis_report view
--   - Added 4 helper functions for analytics queries
--   - Enhanced RLS policies for admins
-- Views: complete_analysis_report
-- Functions: 7 total helper functions
