...
/**
 * Persist embeddings generated by knowledgeEmbedding.service.ts into the
 * `embedding_vector` column of each knowledge_chunk row.
 *
 * Chunks are matched by position (chunk_index order) to the EmbeddingResult
 * array returned by generateEmbeddingsForChunks — both are ordered from 0
 * through N-1 so the positional mapping is correct.
 */
async function persistEmbeddings(
  ...
  if (fetchErr || !dbChunks) {
    warn('7b Embed', `Failed to fetch chunk IDs for embedding persistence: ${fetchErr?.message}`);
    return 0;
  }

  let persisted = 0;
  const limit = Math.min(dbChunks.length, embedResults.length);
  ...
  for (let i = 0; i < limit; i++) {
    ...
    if (updErr) {
      warn('7b Embed', `Failed to persist embedding for chunk ${i}: ${updErr.message}`);
    } else {
      persisted++;
    }
  }
...
async function ingestDocument(
  ...
  try {
    ...
    if (dryRun) {
      ...
      // generateEmbeddingsForChunks returns vectors but does NOT write to DB.
      // This step updates each knowledge_chunk row with its embedding_vector.
      if (documentId && embedResults.length > 0) {
        const t2       = Date.now();
        const persisted = await persistEmbeddings(documentId, embedResults);
